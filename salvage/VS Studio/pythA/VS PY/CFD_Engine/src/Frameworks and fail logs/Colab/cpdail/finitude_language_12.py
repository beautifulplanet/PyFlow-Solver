# -*- coding: utf-8 -*-
"""Finitude Language #12.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1l5fumQ8sjZyC8H6hky3BocEpxpHHDwAv
"""
...
class CorrectedInterpreter:
    ...
    def execute(self, node):
        if node['type'] == 'DECLARATION':
            ...
            # Find the variable in the current or parent scopes and assign
            if node['name'] not in self.current_scope and node['name'] not in self.global_scope:
                 raise RuntimeError(f"Runtime Error: Undeclared variable '{node['name']}'")
            # Prioritize current scope, then global scope
            target_scope = self.current_scope if node['name'] in self.current_scope else self.global_scope
            ...
            if condition_result.get_value() == 1.0:
                for body_node in node['body']:
                    # Stop execution of the block if a return statement is encountered
                    if self._returning:
                        break
                    self.execute(body_node)
...
class CorrectedParser:
    ...
    def consume(self, type):
        ...
        # Improved error reporting with line number and position
        if token:
             # Assuming token has 'lineno' and 'pos' attributes from tokenizer
             raise SyntaxError(f"Syntax Error: Expected '{type}', got '{token['type']}' ('{token['value']}') at line {token['lineno']}, position {token['pos']}")
        else:
             # Estimate position based on the last token or start if no tokens
             last_pos = self.tokens[-1]['pos'] + len(self.tokens[-1]['value']) if self.tokens else 0
    ...
    def parse_statement(self):
        ...
        # New: Handle Function Definition
            ...
            body = []
            while self.current() and self.current()['type'] != 'RBRACE':
                 # Need to handle the ReturnStatement within the body
                 if self.current()['type'] == 'RETURN':
                     body.append(self.parse_return_statement())
                 else:
                     body.append(self.parse_statement())
    ...
    def parse_expression(self):
        ...
        # New: Handle Function Call
            ...
            self.consume('FUNC_CALL') # Consume '!>' token
            func_name = self.consume('IDENTIFIER')['value'] # Consume function name

            # Parse arguments (expressions)
            args = []
            # Assuming arguments are space-separated after the function name for simplicity,
            # until explicit argument delimiters are defined in syntax.
            # Based on the example `:func: my_function <param1 param2> { ... };` and call `!> my_function 10 20;`
            # the arguments are space-separated expressions following the function name.
            # This is different from the parameter syntax '<param1 param2>'.
            # Let's adjust the parsing of arguments based on the example call syntax.
            # The example call syntax `!> my_function 10 20;` suggests arguments are just expressions following the name.
            # This is ambiguous if arguments are complex expressions. Let's assume for now they are simple literals or identifiers.
            # A more robust parser would need explicit delimiters like parentheses `!> my_function (arg1 arg2) ;`
            # Based on the example, let's assume argument parsing continues until a statement ending token (like ';') is encountered,
            # or perhaps until the next operator if in an expression context.
            # The example `!> my_function 10 20;` suggests the function call *is* the expression ending in ';'.
            # Let's assume arguments are zero or more expressions following the function name,
            # stopping before a statement delimiter (like ';') or a known statement start.

            # A simpler approach, given the prefix notation, is that after the function name,
            # the parser expects a number of expressions equal to the function's parameter count.
            # However, the parser doesn't know the function's parameter count yet.
            # The most robust way, aligning with prefix notation and typical AST structures,
            # is for the function call node to contain the function name and a list of argument expressions.
            # The parser should consume expressions until it encounters a token that cannot start an expression
            # (like ';', '}', operators if not the leading token of the expression).
            # Let's refine argument parsing: consume expressions until a SEMICOLON or RBRACE is hit, or until the next token is a known statement start like DECLARE or IF.

            # For the current task, let's strictly follow the example `!> my_function 10 20;`.
            # This implies the arguments are *expressions* immediately following the function name,
            # and the function call itself is an expression that will be followed by a statement ending delimiter like ';'.
            # We need to parse N arguments, where N is not known to the parser.
            # This suggests arguments must be explicitly delimited, or the syntax is simpler.
            # Given the example `:func: my_function <param1 param2> { ... };` and `!> my_function 10 20;`
            # The parameters <param1 param2> are identifiers. The arguments 10 20 are number literals.
            # Let's assume argument parsing consumes expressions until a SEMICOLON is found.
            # This is still problematic in complex expressions like `+|> (!> func1 5) (!> func2 10);`

            # Let's revisit the syntax: `!> my_function 10 20;`. The arguments are just space-separated expressions.
            # The parser needs to parse `10` and `20` as expressions.
            # The most straightforward prefix notation parsing for function calls would be `!> func_name arg1 arg2 arg3 ...`
            # The parser would consume `!>`, then the function name, then call `parse_expression` N times for N arguments.
            # Since the parser doesn't know N, this implies the syntax is not strictly prefix like binary ops,
            # or the argument list needs explicit delimiters.

            # Let's assume for this subtask, arguments are parsed until a token that cannot be part of an expression is found, or until a statement delimiter.
            # This is still complex. Let's simplify based on the minimal example and the TDD principle - implement what's needed for the tests.
            # The expected test cases will guide the exact argument parsing logic.
            # Until we have the test cases defining the call syntax precisely, let's make a simplifying assumption:
            # Arguments are parsed as a sequence of expressions immediately following the function name.
            # The number of arguments must match the function definition's parameter count (checked in interpreter).
            # The parsing of arguments stops when the next token *is* a statement ending token (like ';').

            # Let's try parsing arguments as a list of expressions following the function name.
            # This loop needs a condition to stop.
            # What tokens *always* signal the end of an expression sequence that is part of a function call argument list?
            # In `!> my_function 10 20;`, the ';' ends it.
            # In `~@ res => +|> (!> func_a 5) (!> func_b 10);`, the operators `+|>` and `!=|` (if used in the next part of the expression) signal the end of an argument list *for the inner function call*.

            # Let's assume arguments are expressions parsed sequentially until a token that is NOT a valid start of an expression is found.
            # Valid expression starts: PLUS, MINUS, MULTIPLY, DIVIDE, EQ, NE, GT, LT, GE, LE, NUMBER, IDENTIFIER, FUNC_CALL.
            # Invalid expression starts (in this context): SEMICOLON, ARROW, DECLARE, LBRACE, RBRACE, IF, FUNC_DEF, PARAMS_START, PARAMS_END, RETURN_ARROW, RETURN, LPAREN, RPAREN (unless they are part of a larger expression structure not yet defined).

            # So, consume expressions as arguments while the current token type IS in the set of valid expression start tokens.
            valid_expr_starts = ['PLUS', 'MINUS', 'MULTIPLY', 'DIVIDE', 'EQ', 'NE', 'GT', 'LT', 'GE', 'LE', 'NUMBER', 'IDENTIFIER', 'FUNC_CALL', 'LPAREN'] # Add LPAREN as expressions can be grouped
...
# Keep the CorrectedInterpreter and FinityNumber functions/classes as defined previously
# Assume CorrectedInterpreter, FinityNumber, add_finity, etc. are available from earlier cells
