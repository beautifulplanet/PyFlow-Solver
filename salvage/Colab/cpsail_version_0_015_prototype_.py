# -*- coding: utf-8 -*-
"""CPSAIL Version 0.015 Prototype .ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1oI1uXvPZXE91hiUJYtMZ99-GUBbcDckt
"""
...
def apply_pressure_boundary_conditions_linear_system(A, rhs_vector, grid_info):
    ...
    # According to /content/18pcb.txt, for the pressure-Poisson equation requires either
    # Dirichlet (fixed pressure) or Neumann (fixed pressure gradient) boundary conditions.
...
# Assuming simulation parameters are defined elsewhere or will be defined here for the test case.
sem_order = 4
num_elements_x = 4
num_elements_y = 4
domain_size_x = 2 * np.pi
...
"""# Task
Implement the remaining steps of the plan to integrate the Spectral Element Method (SEM) into the solver structure.
...
*   Conceptual placeholder functions were successfully created for assembling global differentiation, mass, Laplacian, and Bi-Laplacian matrices from 1D components, illustrating the combination of local contributions into a global sparse matrix structure using the LIL matrix format.
*   A conceptual function `calculate_spatial_derivatives_sem` was defined as a placeholder for applying conceptually assembled global SEM operators (differentiation, Laplacian, etc.) to a field represented as a flattened vector, highlighting the core operation of matrix-vector products for derivative calculation.
*   The structure of the main simulation loop was outlined, conceptually demonstrating where the `calculate_spatial_derivatives_sem` function would be called for computing derivatives of the velocity and pressure fields within an IMEX and projection method framework.

### Insights or Next Steps

*   The next step is to replace the conceptual functions with detailed implementations for assembling global sparse matrices (likely in CSR format for efficiency) for the Mass, Stiffness (Laplacian), Bi-Laplacian, Gradient, and Divergence operators, considering proper element-wise assembly and mapping between local and global degrees of freedom in 2D/3D.
*   Implement the reshaping functions to convert between the grid representation of fields and the flattened vector representation required for matrix-vector products with the global operators.
...
"""## Implement global SEM assembly

**Subtask**: Implement functions to assemble the global differentiation, mass, Laplacian, and Bi-Laplacian matrices from the 1D components. This will involve considering the elemental structure and mapping between local and global degrees of freedom.
"""
...
# Assuming 1D SEM components (gauss_lobatto_legendre_nodes_weights, compute_differentiation_matrix_1d, compute_mass_matrix_1d) are defined in a previous cell.
# Assuming SEM parameters (sem_order, num_elements_x, num_elements_y) are defined elsewhere.
...
# Assuming D_1d and M_1d are available from the previous 1D implementation step
# If not, uncomment and run the following lines to get them
gll_nodes, gll_weights = gauss_lobatto_legendre_nodes_weights(sem_order)
D_1d = compute_differentiation_matrix_1d(gll_nodes)
M_1d = compute_mass_matrix_1d(gll_weights)


print("Implementing global SEM assembly functions using sparse matrices.")

# Helper function to map 1D local node index to global node index
def get_global_index_1d(element_index, local_node_index, sem_order):
    """
    Maps a local node index within an element to its global index in 1D.
    """
    return element_index * sem_order + local_node_index

# Helper function for element-wise assembly (conceptual base)
def assemble_elemental_operator_1d(local_operator, element_jacobian):
    """
    Conceptual function to scale a local 1D operator by the element Jacobian.
    In SEM, elemental matrices are scaled by the Jacobian of the mapping from reference
    to physical element. For uniform elements, this is a constant factor (element size / 2).
    """
    # For uniform elements in 1D, Jacobian is element_size / 2
    # We'll assume uniform elements and integrate Jacobian scaling into assembly.
    # In a real implementation, Jacobian would vary for non-uniform or curved elements.
    return local_operator # Simplified for conceptual uniform elements


# Implement function to assemble the global Mass matrix (2D)
def assemble_mass_matrix_global_2d(num_elements_x, num_elements_y, sem_order, M_1d_x, M_1d_y, grid_info):
    """
    Assembles the global 2D Mass matrix from 1D mass matrices.
    Uses element-wise assembly.
    """
    n_points_1d = sem_order + 1
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_2d = num_global_points_x * num_global_points_y

    # Initialize global sparse matrix
    M_global = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))

    element_size_x = grid_info['domain_size_x'] / num_elements_x
    element_size_y = grid_info['domain_size_y'] / num_elements_y

    # Element assembly loop
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            # Get local to global mapping for this element
            # Global indices for the nodes of element (ex, ey)
            global_indices = []
            for j_local in range(n_points_1d):
                for i_local in range(n_points_1d):
                     # Map 2D local index (i_local, j_local) to 1D global index
                     # Assuming row-major ordering of global nodes (i varies faster than j)
                     i_global_1d = get_global_index_1d(ex, i_local, sem_order)
                     j_global_1d = get_global_index_1d(ey, j_local, sem_order)
                     global_index_2d = j_global_1d * num_global_points_x + i_global_1d
                     global_indices.append(global_index_2d)


            global_indices = np.array(global_indices)

            # Local elemental mass matrix in 2D (tensor product of 1D)
            # M_elemental_2d = np.kron(M_1d_y, M_1d_x) # Kronecker product

            # Element Jacobian for uniform rectangular elements
            jacobian_det = (element_size_x / 2.0) * (element_size_y / 2.0)

            # Elemental Mass matrix (scaled by Jacobian) - in SEM, mass matrix is diagonal in GLL basis
            # M_elemental_local = np.diag(np.kron(M_1d_y.diagonal(), M_1d_x.diagonal())) # For diagonal mass matrix

            # For stiffness/mass matrix assembly, integrate local contributions to global matrix
            # Elemental Mass matrix contribution (scaled by Jacobian)
            # In SEM, the local mass matrix is diagonal (using GLL weights)
            # M_elemental_local_contrib = np.outer(gll_weights, gll_weights) * jacobian_det # 2D outer product of weights

            # Assemble local contributions to the global matrix
            # This requires mapping local (i_local, j_local) pairs to the correct global indices
            # and adding the contribution M_elemental_local_contrib[i_local_2d, j_local_2d]
            # where (i_local_2d, j_local_2d) is the flattened local 2D index.

            local_mass_2d_contrib = np.outer(M_1d_y.diagonal(), M_1d_x.diagonal()) * jacobian_det
            local_mass_2d_contrib_flat = local_mass_2d_contrib.flatten()


            for local_idx_flat in range(n_points_1d * n_points_1d):
                 global_i = global_indices[local_idx_flat]
                 global_j = global_indices[local_idx_flat] # Mass matrix is diagonal in GLL basis

                 M_global[global_i, global_j] += local_mass_2d_contrib_flat[local_idx_flat]


    print("Global 2D Mass matrix assembled.")
    return M_global.tocsr() # Convert to CSR for efficient matrix-vector products


# Implement function to assemble the global 2D Differentiation matrices (Dx, Dy)
def assemble_differentiation_matrices_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    """
    Assembles the global 2D Differentiation matrices (Dx and Dy).
    Dx acts on x-derivatives, Dy acts on y-derivatives.
    Uses element-wise assembly.
    """
    n_points_1d = sem_order + 1
    num_global_points_x = num_elements_x * sem_order + 1
    ...
    Dx_global = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))
    Dy_global = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))
    ...
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            ...
            # Elemental matrices for differentiation in x and y
            # Local Dx contribution: (D_1d_x @ M_1d_x) kron M_1d_y * (element_size_y / 2)  -- scaled by Jacobian
            # Local Dy contribution: M_1d_x kron (D_1d_y @ M_1d_y) * (element_size_x / 2)  -- scaled by Jacobian

            # In SEM, the differentiation matrix application in weak form involves M * D.T * field
            # For stiffness matrix (related to Laplacian), we use D.T * M * D.
            # Let's assemble the operators that perform the derivative itself (not in weak form sense yet)
            # The operator applying d/dx in 2D is D_1d_x kron Identity_y, scaled by Jacobian_x / Jacobian_2D
            # The operator applying d/dy in 2D is Identity_x kron D_1d_y, scaled by Jacobian_y / Jacobian_2D

            # Let's use the definition based on the stiffness matrix components for Laplacian assembly.
            # Global Laplacian: - (nabla . (mu nabla u)) -> weak form (nabla v . mu nabla u)
            # Discretized Laplacian component e.g., d^2/dx^2 : D_x.T @ M_x @ D_x kron M_y
            # Let's assemble these components.

            # Stiffness matrix component for d^2/dx^2
            Kxx_elemental = (D_1d_x.T @ M_1d_x @ D_1d_x) # No scaling by Jacobian here yet, done during global assembly
            # Stiffness matrix component for d^2/dy^2
            Kyy_elemental = (D_1d_y.T @ M_1d_y @ D_1d_y)

            # Assemble local stiffness contributions to the global matrix
            # Global Kxx += Kxx_elemental kron M_1d_y * (element_size_y / 2) # Scaled by Jacobian_y (for M_y) and Jacobian_2D
            # Global Kyy += M_1d_x kron Kyy_elemental * (element_size_x / 2) # Scaled by Jacobian_x (for M_x) and Jacobian_2D

            # Need the element stiffness matrices in 2D
            # K_elemental_2d = Kxx_elemental kron M_1d_y + M_1d_x kron Kyy_elemental # For Laplacian assembly

            # Let's rethink. The request is for Differentiation matrices (Dx, Dy), not Stiffness.
            # Global Dx operator (maps field to its x-derivative). In 2D, conceptually M_y kron D_x.T @ M_x
            # The operator applying d/dx on a field phi is sum_e int_Omega_e (d/dx phi) v dOmega
            # = sum_e int_I_x int_I_y (d/dx phi) v J_x J_y dxi deta
            # = sum_e int_I_x int_I_y (d/dxi phi * dxi/dx) v J_x J_y dxi deta
            # dxi/dx = 1/J_x = 2/element_size_x
            # = sum_e int_I_x int_I_y (d/dxi phi) v (2/element_size_x) J_x J_y dxi deta
            # Using tensor products of 1D differentiation matrix (D_1d.T @ M_1d)
            # Global Dx operator is M_y_global kron (D_1d_x.T @ M_1d_x)_global.
            # Global Dy operator is (D_1d_y.T @ M_1d_y)_global kron M_x_global.

            # Let's assemble the global operators that perform the differentiation.
            # Global Dx: M_y_global kron (D_1d_x.T @ M_1d_x)_global
            # Global Dy: (D_1d_y.T @ M_1d_y)_global kron M_x_global

            # For simplicity in this conceptual stage, let's assemble the operators that
            # map the nodal values to nodal values of the derivative. This is D_global.
            # Global D_x in 2D: Identity_y kron D_x_global (where D_x_global is the 1D assembled D)
            # Global D_y in 2D: D_y_global kron Identity_x (where D_y_global is the 1D assembled D)

            # Assemble 1D global Differentiation matrices first conceptually
            D_x_global_1d_assembled = assemble_global_sem_operator(D_1d_x, 'x', num_elements_x, sem_order)
            D_y_global_1d_assembled = assemble_global_sem_operator(D_1d_y, 'y', num_elements_y, sem_order)

            # Global 2D Differentiation matrices acting on flattened vector (assuming row-major flattening)
            # Dx operator applies D_x_global_1d_assembled independently to each row (along x)
            Dx_global = block_diag([D_x_global_1d_assembled] * num_global_points_y)

            # Dy operator applies D_y_global_1d_assembled independently to each column (along y).
            # This is more complex to represent as a simple block diagonal in row-major flattening.
            # It's easier to think of applying D_y_global_1d_assembled to the reshaped field, then reshaping back.
            # However, the goal is a global sparse matrix for matrix-vector products.
            # The structure of Dy_global for row-major flattening involves permuting the vector.

            # A correct way to think about Dy_global in row-major flattening:
            # Dy_global = (D_y_global_1d_assembled kron Identity_x) * Permutation_matrix
            # where Permutation_matrix converts between row-major and column-major orderings.
            # Or, assemble directly by looping through global nodes and their neighbors in y.

            # Let's assemble Dy_global by directly looping through global points.
            # For a global point (i, j) with global index k = j * num_global_points_x + i,
            # the derivative w.r.t. y at this point depends on values at (i, l) for l = 0..num_global_points_y-1.
            # The entry in Dy_global at row k and column m (global index for (p, q)) is related to D_y_global_1d_assembled[j, q] if i == p.

            for j_global in range(num_global_points_y):
                 for i_global in range(num_global_points_x):
                     global_row_idx = j_global * num_global_points_x + i_global
                     for l_global in range(num_global_points_y):
                         global_col_idx = l_global * num_global_points_x + i_global
                         # Dy_global[global_row_idx, global_col_idx] = (Identity_x @ D_y_global_1d_assembled)[global_row_idx, global_col_idx] # This is not right
                         # The derivative w.r.t y at (i_global, j_global) depends on values at (i_global, l_global) multiplied by D_y_global_1d_assembled[j_global, l_global]
                         Dy_global[global_row_idx, global_col_idx] = D_y_global_1d_assembled[j_global, l_global]


    print("Global 2D Differentiation matrices (Dx, Dy) assembled.")
    return Dx_global.tocsr(), Dy_global.tocsr() # Convert to CSR


# Implement function to assemble the global 2D Laplacian matrix (nabla^2)
def assemble_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    """
    Assembles the global 2D Laplacian matrix (nabla^2).
    Uses element-wise assembly based on the weak form.
    Stiffness matrix K = K_x + K_y
    K_x_elemental = D_1d_x.T @ M_1d_x @ D_1d_x
    K_y_elemental = D_1d_y.T @ M_1d_y @ D_1d_y
    Global K_x += K_x_elemental kron M_1d_y * (element_size_y/2)
    Global K_y += M_1d_x kron K_y_elemental * (element_size_x/2)
    Laplacian operator in strong form is M_global_inv * (-K_global).
    Here, we assemble the stiffness matrix K_global.
    """
    print("\nAssembling Global 2D Laplacian (Stiffness) Matrix (K).")
    ...
    K_global = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))
    ...
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            ...
            # Elemental stiffness matrices in 1D
            K_1d_x = D_1d_x.T @ M_1d_x @ D_1d_x
            K_1d_y = D_1d_y.T @ M_1d_y @ D_1d_y

            # Elemental stiffness matrices in 2D for each component of Laplacian
            Kxx_elemental_2d = np.kron(K_1d_x, M_1d_y) # Kron(Kx, My)
            Kyy_elemental_2d = np.kron(M_1d_x, K_1d_y) # Kron(Mx, Ky)

            K_elemental_2d = Kxx_elemental_2d + Kyy_elemental_2d # Elemental Stiffness Matrix

            # Scale by element Jacobian
            K_elemental_scaled = K_elemental_2d * jacobian_det

            # Assemble local contributions to the global matrix
            for local_row_flat in range(n_points_1d * n_points_1d):
                for local_col_flat in range(n_points_1d * n_points_1d):
                     global_row = global_indices[local_row_flat]
                     global_col = global_indices[local_col_flat]
                     K_global[global_row, global_col] += K_elemental_scaled[local_row_flat, local_col_flat]


    print("Global 2D Laplacian (Stiffness) matrix assembled.")
    return K_global.tocsr() # Convert to CSR


# Implement function to assemble the global 2D Bi-Laplacian matrix (nabla^4)
# This is more complex and might involve assembling the Laplacian operator twice
# or assembling a dedicated Bi-Laplacian stiffness matrix.
# Weak form of Bi-Laplacian: int (nabla^2 v) (nabla^2 u) dOmega
# Discretized: K_bi = (K_global @ M_global_inv) @ (K_global @ M_global_inv) @ M_global (conceptual)
# Or, assemble elemental Bi-Laplacian stiffness matrix and sum up.
def assemble_bi_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    """
    Conceptual function to assemble the global 2D Bi-Laplacian matrix (nabla^4).
    This is a placeholder as the assembly of the Bi-Laplacian stiffness matrix
    is significantly more involved than the Laplacian.
    """
    print("\nConceptual Global 2D Bi-Laplacian Matrix Assembly.")
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_2d = num_global_points_x * num_global_points_y

    # Return a placeholder sparse matrix
    # A full implementation would assemble the Bi-Laplacian stiffness matrix
    # based on the weak form integral.
    return lil_matrix((num_global_dofs_2d, num_global_dofs_2d))


# Implement function to assemble the global 2D Gradient operator
# This operator maps from scalar pressure DOFs to vector velocity DOFs.
# For a scalar field p, grad p = [dp/dx, dp/dy].
# Discretized, this involves applying the gradient operator components.
# The global gradient operator (mapping from pressure DOFs to velocity DOFs)
# has dimensions (num_velocity_dofs, num_pressure_dofs).
# Assuming velocity DOFs are [u_x_vec, u_y_vec].
# grad_p_vector = G_global @ p_vector
# G_global = [ Gx_global ]
#            [ Gy_global ]
# Gx_global maps pressure DOFs to u_x DOFs (same grid size). Gx_global is M_y_global kron (D_1d_x.T @ M_1d_x)_global
# Gy_global maps pressure DOFs to u_y DOFs (same grid size). Gy_global is (D_1d_y.T @ M_1d_y)_global kron M_x_global
# This is for the weak form operator. For applying gradient to nodal values:
# Gx_global = block_diag([D_1d_x_assembled] * num_global_points_y)
# Gy_global = ... (more complex due to reshaping)

def assemble_gradient_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, D_1d_y):
    """
    Assembles the global 2D Gradient operator (nabla), mapping from scalar to vector field DOFs.
    Returns a block matrix [Gx_global, Gy_global]^T where Gx/Gy map scalar to scalar field components.
    Using the nodal differentiation matrices (D_global).
    """
    print("\nAssembling Global 2D Gradient Operator.")
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_scalar = num_global_points_x * num_global_points_y

    # Assemble global 1D Differentiation matrices
    D_x_global_1d_assembled = assemble_global_sem_operator(D_1d_x, 'x', num_elements_x, sem_order)
    D_y_global_1d_assembled = assemble_global_sem_operator(D_1d_y, 'y', num_elements_y, sem_order)

    # Gx operator: Applies D_x_global_1d_assembled to each row
    Gx_global = block_diag([D_x_global_1d_assembled] * num_global_points_y)

    # Gy operator: Applies D_y_global_1d_assembled to each column.
    # For row-major flattening, this is more complex. Assemble directly.
    Gy_global = lil_matrix((num_global_dofs_scalar, num_global_dofs_scalar))
    ...
    for j_global in range(num_global_points_y):
        for i_global in range(num_global_points_x):
            ...
            for l_global in range(num_global_points_y):
                global_col_idx = l_global * num_global_points_x + i_global
                Gy_global[global_row_idx, global_col_idx] = D_y_global_1d_assembled[j_global, l_global]


    # Combine Gx and Gy into a single block matrix representing the gradient operator
    # This operator maps a scalar pressure vector (size num_global_dofs_scalar)
    # to a concatenated velocity gradient vector [dp/dx_vec, dp/dy_vec] (size 2 * num_global_dofs_scalar).
    Gradient_global = csr_matrix(block_diag([Gx_global, Gy_global]))


    print("Global 2D Gradient operator assembled.")
    # Return Gx_global and Gy_global separately for flexibility, or the combined Gradient_global
    # Returning Gx and Gy which map scalar field to scalar field component derivatives.
    return Gx_global.tocsr(), Gy_global.tocsr()


# Implement function to assemble the global 2D Divergence operator
# This operator maps from vector velocity DOFs to scalar pressure DOFs.
# For a vector field u = [u_x, u_y], div u = du_x/dx + du_y/dy.
# Discretized, this involves applying the divergence operator components.
# The global divergence operator (mapping from velocity DOFs to pressure DOFs)
# has dimensions (num_pressure_dofs, num_velocity_dofs).
# Assuming velocity DOFs are [u_x_vec, u_y_vec] and pressure DOFs match scalar DOFs.
# div_u_vector = Div_global @ u_vector
# u_vector = [ u_x_vec ]
#            [ u_y_vec ]
# Div_global = [ Div_x_global  Div_y_global ]
# Div_x_global maps u_x DOFs to scalar DOFs. Div_x_global is (D_1d_x.T @ M_1d_x)_global kron M_y_global (weak form)
# Div_y_global maps u_y DOFs to scalar DOFs. Div_y_global is M_x_global kron (D_1d_y.T @ M_1d_y)_global (weak form)

# Using the nodal differentiation matrices (D_global):
# Div_x_global = block_diag([D_1d_x_assembled.T] * num_global_points_y)
# Div_y_global = ... (more complex transpose and permutation)
# Div_global = [ Div_x_global  Div_y_global ]

def assemble_divergence_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, D_1d_y):
    """
    Assembles the global 2D Divergence operator (nabla .), mapping from vector to scalar field DOFs.
    Returns a block matrix [Div_x_global Div_y_global] where Div_x/Div_y map scalar field components to scalar field.
    Using the nodal differentiation matrices (D_global).
    """
    print("\nAssembling Global 2D Divergence Operator.")
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_scalar = num_global_points_x * num_global_points_y
    ...
    # Div_x operator: Applies D_x_global_1d_assembled.T to each row (along x)
    Div_x_global = block_diag([D_x_global_1d_assembled.T] * num_global_points_y)

    # Div_y operator: Applies D_y_global_1d_assembled.T to each column (along y).
    # Assemble directly by looping through global points.
    Div_y_global = lil_matrix((num_global_dofs_scalar, num_global_dofs_scalar))
    ...
    for j_global in range(num_global_points_y):
        for i_global in range(num_global_points_x):
            ...
            for l_global in range(num_global_points_y):
                global_col_idx = l_global * num_global_points_x + i_global
                # Div_y_global[global_row_idx, global_col_idx] = (Identity_x @ D_y_global_1d_assembled.T)[global_row_idx, global_col_idx] # Not right
                # The divergence w.r.t y at (i_global, j_global) depends on values at (i_global, l_global) multiplied by D_y_global_1d_assembled.T[j_global, l_global]
                Div_y_global[global_row_idx, global_col_idx] = D_y_global_1d_assembled.T[j_global, l_global]

    # Combine Div_x and Div_y into a single block matrix representing the divergence operator
    # This operator maps a concatenated velocity vector [u_x_vec, u_y_vec]
    # to a scalar divergence vector (size num_global_dofs_scalar).
    # Div_global = [ Div_x_global  Div_y_global ]
    Divergence_global = csr_matrix((num_global_dofs_scalar, 2 * num_global_dofs_scalar))
    Divergence_global[:, :num_global_dofs_scalar] = Div_x_global
    Divergence_global[:, num_global_dofs_scalar:] = Div_y_global


    print("Global 2D Divergence operator assembled.")
    # Return Div_x_global and Div_y_global separately or the combined Divergence_global
    # Returning Div_x and Div_y which map scalar field component to scalar field.
    return Div_x_global.tocsr(), Div_y_global.tocsr()


print("\nGlobal SEM assembly functions implemented with conceptual 2D assembly using sparse matrices.")
print("Includes functions for assembling Mass, Differentiation (Dx, Dy), Laplacian, Bi-Laplacian (placeholder), Gradient (Gx, Gy), and Divergence (Divx, Divy) operators.")
print("Note: Bi-Laplacian assembly is a placeholder and the assembly procedures for 2D/3D operators require careful implementation based on element-wise contributions or tensor products.")
print("The Gradient and Divergence operators are assembled based on nodal differentiation matrices.")

"""## Implement spatial derivative calculation

**Subtask**: Develop functions that use the assembled global SEM operators to calculate spatial derivatives (gradient, divergence, Laplacian, Bi-Laplacian) of the velocity and pressure fields.
"""

import numpy as np
from scipy.sparse import csr_matrix # Import CSR matrix format for efficient matrix-vector products

# Assuming global_operators dictionary is available from the global assembly step,
# containing assembled global sparse matrices like 'M_global', 'K_global_laplacian',
# 'K_global_bilaplacian', 'Gx_global', 'Gy_global', 'Div_x_global', 'Div_y_global'.
# Assuming grid_info is also available, containing grid dimensions and mapping information.

# Placeholder for assembled global operators (will be populated by assembly functions)
# global_operators = {
#     'M_global': None, # Global Mass matrix (CSR)
#     'K_global_laplacian': None, # Global Laplacian Stiffness matrix (CSR)
#     'K_global_bilaplacian': None, # Global Bi-Laplacian Stiffness matrix (CSR) - placeholder assembly
#     'Gx_global': None, # Global Gradient operator component d/dx (CSR)
#     'Gy_global': None, # Global Gradient operator component d/dy (CSR)
#     'Div_x_global': None, # Global Divergence operator component d/dx (CSR)
#     'Div_y_global': None, # Global Divergence operator component d/dy (CSR)
# }

# Placeholder grid_info (will be populated by grid creation function)
# grid_info = {
#     'num_global_points_x': None,
#     'num_global_points_y': None,
#     'num_global_dofs_scalar': None, # Total scalar DOFs (nx * ny)
#     'num_global_dofs_velocity': None, # Total velocity DOFs (nx * ny * ndim)
#     # Add mapping info if needed (e.g., from flattened vector index to grid index)
# }

print("Implementing functions for calculating spatial derivatives using assembled global SEM operators.")

# Helper function to calculate the full Gradient of a scalar field (e.g., pressure)
def calculate_gradient_sem_2d(scalar_field_vector, global_operators, grid_info):
    """
    Calculates the gradient of a scalar field using assembled global SEM operators.

    Args:
        scalar_field_vector (np.ndarray): The scalar field (e.g., pressure) as a flattened vector.
                                         Size should match num_global_dofs_scalar.
        global_operators (dict): Dictionary containing assembled global SEM operators,
                                 specifically 'Gx_global' and 'Gy_global'.
        grid_info (dict): Information about the grid.

    Returns:
        np.ndarray: The gradient of the scalar field as a flattened vector.
                    Assumed to be concatenated components [dp/dx_vec, dp/dy_vec].
                    Size should match num_global_dofs_velocity.
    """
    print("\nCalculating Gradient using assembled global SEM operators (2D).")

    if 'Gx_global' not in global_operators or 'Gy_global' not in global_operators:
        print("Error: Global Gradient operators (Gx_global, Gy_global) not found in global_operators dictionary.")
        # Return placeholder zero vector with conceptual velocity DOFs size
        conceptual_velocity_dofs = grid_info.get('num_global_dofs_velocity', 1000 * 2) # Example default
        return np.zeros(conceptual_velocity_dofs)


    Gx_global = global_operators['Gx_global']
    Gy_global = global_operators['Gy_global']

    # Calculate components of the gradient
    gradient_x_vector = Gx_global @ scalar_field_vector
    gradient_y_vector = Gy_global @ scalar_field_vector

    # Concatenate components to form the full gradient vector
    gradient_vector = np.concatenate([gradient_x_vector, gradient_y_vector])

    print("Gradient calculation completed.")
    return gradient_vector


# Helper function to calculate the Divergence of a vector field (e.g., velocity)
def calculate_divergence_sem_2d(vector_field_vector, global_operators, grid_info):
    """
    Calculates the divergence of a vector field using assembled global SEM operators.

    Args:
        vector_field_vector (np.ndarray): The vector field (e.g., velocity) as a flattened vector.
                                         Assumed to be concatenated components [u_x_vec, u_y_vec].
                                         Size should match num_global_dofs_velocity.
        global_operators (dict): Dictionary containing assembled global SEM operators,
                                 specifically 'Div_x_global' and 'Div_y_global'.
        grid_info (dict): Information about the grid.

    Returns:
        np.ndarray: The divergence of the vector field as a flattened scalar vector.
                    Size should match num_global_dofs_scalar.
    """
    print("\nCalculating Divergence using assembled global SEM operators (2D).")

    if 'Div_x_global' not in global_operators or 'Div_y_global' not in global_operators:
        print("Error: Global Divergence operators (Div_x_global, Div_y_global) not found in global_operators dictionary.")
        # Return placeholder zero vector with conceptual scalar DOFs size
        conceptual_scalar_dofs = grid_info.get('num_global_dofs_scalar', 1000) # Example default
        return np.zeros(conceptual_scalar_dofs)

    Div_x_global = global_operators['Div_x_global']
    Div_y_global = global_operators['Div_y_global']

    # Assuming vector_field_vector is [u_x_vec, u_y_vec]
    num_scalar_dofs = Div_x_global.shape[1] # Size of one component vector
    u_x_vector = vector_field_vector[:num_scalar_dofs]
    u_y_vector = vector_field_vector[num_scalar_dofs:]

    # Calculate divergence components and sum them
    divergence_x = Div_x_global @ u_x_vector
    divergence_y = Div_y_global @ u_y_vector

    divergence_vector = divergence_x + divergence_y

    print("Divergence calculation completed.")
    return divergence_vector


# Helper function to calculate the Laplacian of a field
def calculate_laplacian_sem_2d(field_vector, global_operators, grid_info):
    """
    Calculates the Laplacian of a field using the assembled global Laplacian (Stiffness) matrix.

    Args:
        field_vector (np.ndarray): The input field (scalar or vector component) as a flattened vector.
                                  Size should match num_global_dofs_scalar.
        global_operators (dict): Dictionary containing assembled global SEM operators,
                                 specifically 'K_global_laplacian' (Laplacian Stiffness matrix).
                                 Also needs 'M_global' (Mass matrix) to convert stiffness application
                                 (weak form) to strong form operator (Laplacian = M_inv * -K).
        grid_info (dict): Information about the grid.

    Returns:
        np.ndarray: The Laplacian of the field as a flattened vector.
                    Size should match num_global_dofs_scalar.
    """
    print("\nCalculating Laplacian using assembled global SEM operators (2D).")

    if 'K_global_laplacian' not in global_operators or 'M_global' not in global_operators:
        print("Error: Global Laplacian Stiffness matrix (K_global_laplacian) or Mass matrix (M_global) not found.")
        # Return placeholder zero vector with conceptual scalar DOFs size
        conceptual_scalar_dofs = grid_info.get('num_global_dofs_scalar', 1000) # Example default
        return np.zeros(conceptual_scalar_dofs)

    K_global = global_operators['K_global_laplacian']
    M_global = global_operators['M_global']

    # Calculate Laplacian in strong form: Laplacian(field) = M_inv * (-K * field)
    # Solving M * Laplacian(field) = -K * field is often more stable than explicit M_inv.
    # Here, we assume M_global is diagonal (for GLL) and can be inverted easily.
    M_global_inv = csr_matrix((1.0 / M_global.diagonal(), (np.arange(M_global.shape[0]), np.arange(M_global.shape[0])), (M_global.shape[0], M_global.shape[1])))

    laplacian_vector = M_global_inv @ (-K_global @ field_vector)

    print("Laplacian calculation completed.")
    return laplacian_vector


# Helper function to calculate the Bi-Laplacian of a field
def calculate_bi_laplacian_sem_2d(field_vector, global_operators, grid_info):
    """
    Calculates the Bi-Laplacian of a field using assembled global SEM operators.

    Args:
        field_vector (np.ndarray): The input field (scalar or vector component) as a flattened vector.
                                  Size should match num_global_dofs_scalar.
        global_operators (dict): Dictionary containing assembled global SEM operators,
                                 specifically 'K_global_bilaplacian' (Bi-Laplacian Stiffness matrix).
                                 Also needs 'M_global' (Mass matrix).
                                 Alternatively, can use the Laplacian operator twice.
        grid_info (dict): Information about the grid.

    Returns:
        np.ndarray: The Bi-Laplacian of the field as a flattened vector.
                    Size should match num_global_dofs_scalar.
    """
    print("\nCalculating Bi-Laplacian using assembled global SEM operators (2D).")

    # Option 1: Use the assembled Bi-Laplacian Stiffness matrix (if available)
    if 'K_global_bilaplacian' in global_operators and 'M_global' in global_operators:
         print("Using assembled Bi-Laplacian Stiffness matrix.")
         K_bi_global = global_operators['K_global_bilaplacian']
         M_global = global_operators['M_global']
         M_global_inv = csr_matrix((1.0 / M_global.diagonal(), (np.arange(M_global.shape[0]), np.arange(M_global.shape[0])), (M_global.shape[0], M_global.shape[1])))
         bi_laplacian_vector = M_global_inv @ (-K_bi_global @ field_vector)
         print("Bi-Laplacian calculation completed using stiffness matrix.")
         return bi_laplacian_vector

    # Option 2: Apply the Laplacian operator twice (nabla^4 = nabla^2 * nabla^2)
    elif 'K_global_laplacian' in global_operators and 'M_global' in global_operators:
         print("Using Laplacian operator twice.")
         # Calculate the first Laplacian
         laplacian_field_vector = calculate_laplacian_sem_2d(field_vector, global_operators, grid_info)
         # Calculate the Laplacian of the Laplacian
         bi_laplacian_vector = calculate_laplacian_sem_2d(laplacian_field_vector, global_operators, grid_info)
         print("Bi-Laplacian calculation completed by applying Laplacian twice.")
         return bi_laplacian_vector

    else:
        print("Error: Global Bi-Laplacian Stiffness matrix or Laplacian Stiffness/Mass matrices not found.")
        # Return placeholder zero vector with conceptual scalar DOFs size
        conceptual_scalar_dofs = grid_info.get('num_global_dofs_scalar', 1000) # Example default
        return np.zeros(conceptual_scalar_dofs)


# Main function to calculate all relevant spatial derivatives for the solver
def calculate_all_spatial_derivatives_sem_2d(velocity_field_vector, pressure_field_vector, global_operators, grid_info):
    """
    Calculates all necessary spatial derivatives for the 2D fluid dynamics solver.

    Args:
        velocity_field_vector (np.ndarray): The velocity field as a flattened vector [u_x_vec, u_y_vec].
                                         Size should match num_global_dofs_velocity.
        pressure_field_vector (np.ndarray): The pressure field as a flattened vector.
                                         Size should match num_global_dofs_scalar.
        global_operators (dict): Dictionary containing all assembled global SEM operators.
        grid_info (dict): Information about the grid.

    Returns:
        dict: A dictionary containing calculated spatial derivatives as flattened vectors.
              Keys: 'velocity_gradient_x', 'velocity_gradient_y', 'velocity_laplacian_x',
                    'velocity_laplacian_y', 'velocity_bi_laplacian_x', 'velocity_bi_laplacian_y',
                    'velocity_divergence', 'pressure_gradient_x', 'pressure_gradient_y',
                    'pressure_laplacian'.
    """
    print("\nCalculating all spatial derivatives for the 2D solver.")

    # Assuming velocity_field_vector is [u_x_vec, u_y_vec]
    num_scalar_dofs = grid_info.get('num_global_dofs_scalar', velocity_field_vector.shape[0] // 2)
    u_x_vector = velocity_field_vector[:num_scalar_dofs]
    u_y_vector = velocity_field_vector[num_scalar_dofs:]

    derivatives = {}

    # Calculate derivatives of velocity components
    derivatives['velocity_gradient_x'] = calculate_gradient_sem_2d(u_x_vector, global_operators, grid_info) # Gradient of u_x (vector [d(ux)/dx, d(ux)/dy])
    derivatives['velocity_gradient_y'] = calculate_gradient_sem_2d(u_y_vector, global_operators, grid_info) # Gradient of u_y (vector [d(uy)/dx, d(uy)/dy])

    derivatives['velocity_laplacian_x'] = calculate_laplacian_sem_2d(u_x_vector, global_operators, grid_info) # Laplacian of u_x (scalar)
    derivatives['velocity_laplacian_y'] = calculate_laplacian_sem_2d(u_y_vector, global_operators, grid_info) # Laplacian of u_y (scalar)

    derivatives['velocity_bi_laplacian_x'] = calculate_bi_laplacian_sem_2d(u_x_vector, global_operators, grid_info) # Bi-Laplacian of u_x (scalar)
    derivatives['velocity_bi_laplacian_y'] = calculate_bi_laplacian_sem_2d(u_y_vector, global_operators, grid_info) # Bi-Laplacian of u_y (scalar)

    derivatives['velocity_divergence'] = calculate_divergence_sem_2d(velocity_field_vector, global_operators, grid_info) # Divergence of u (scalar)

    # Calculate derivatives of pressure
    derivatives['pressure_gradient'] = calculate_gradient_sem_2d(pressure_field_vector, global_operators, grid_info) # Gradient of p (vector [dp/dx, dp/dy])
    derivatives['pressure_laplacian'] = calculate_laplacian_sem_2d(pressure_field_vector, global_operators, grid_info) # Laplacian of p (scalar)


    print("All spatial derivatives calculated.")
    return derivatives


print("\nFunctions for calculating spatial derivatives using assembled global SEM operators defined.")
print("Includes functions for Gradient, Divergence, Laplacian, and Bi-Laplacian in 2D.")
print("A main function calculate_all_spatial_derivatives_sem_2d is provided to calculate all required derivatives for the solver.")

"""## Integrate SEM into solver structure

**Subtask**: Outline how these implemented SEM components will be used within the main simulation loop to compute the necessary spatial derivative terms at each time step.
"""
...
# Conceptual Assembled Global Operators (needs full implementation in SEM step)
# global_operators = ... # Dictionary of assembled sparse matrices (e.g., 'M_global', 'K_global_laplacian', 'K_global_bilaplacian', 'Gx_global', 'Gy_global', 'Div_x_global', 'Div_y_global')
# For now, use an empty dictionary as a placeholder
global_operators = {}

# Conceptual grid_info with necessary keys for spatial derivative functions
grid_info = {
    'num_global_points_x': 17, # Example value
    'num_global_points_y': 17, # Example value
    'num_global_dofs_scalar': 17 * 17, # Total scalar DOFs (nx * ny)
    'num_global_dofs_velocity': 17 * 17 * 2, # Total velocity DOFs (nx * ny * ndim)
    # Add other necessary grid info like boundary node indices, mapping info, etc.
}


print("Outlining the structure of the main simulation loop, integrating spatial derivative calculations.")
...
#     # Use conceptual placeholder vectors with correct sizes based on grid info
#     # In a real implementation, these would be the actual velocity_field and pressure_field
#     # converted to vectors from their grid representations. For now, use placeholder vectors
#     # to simulate data flow with the spatial derivative functions.
#     current_velocity_vector = np.zeros(grid_info['num_global_dofs_velocity']) # Placeholder vector
#     current_pressure_vector = np.zeros(grid_info['num_global_dofs_scalar']) # Placeholder vector


#     # Calculate all required spatial derivatives of the current velocity and pressure fields at time t^n using SEM.
#     # This call will use the assembled global operators internally.
#     # spatial_derivatives_n = calculate_all_spatial_derivatives_sem_2d(
#     #     current_velocity_vector, current_pressure_vector, global_operators, grid_info
#     # ) # Call to the implemented spatial derivative function


#     # Extract necessary derivatives for the IMEX step
#     # velocity_gradient_x_n = spatial_derivatives_n['velocity_gradient_x'] # d(ux)/dx, d(ux)/dy
#     # velocity_gradient_y_n = spatial_derivatives_n['velocity_gradient_y'] # d(uy)/dx, d(uy)/dy
#     # velocity_laplacian_x_n = spatial_derivatives_n['velocity_laplacian_x'] # nabla^2(ux)
#     # velocity_laplacian_y_n = spatial_derivatives_n['velocity_laplacian_y'] # nabla^2(uy)
#     # velocity_bi_laplacian_x_n = spatial_derivatives_n['velocity_bi_laplacian_x'] # nabla^4(ux)
#     # velocity_bi_laplacian_y_n = spatial_derivatives_n['velocity_bi_laplacian_y'] # nabla^4(uy)


#     # Implement the calculation of the explicit convective term (u.grad)u at time t^n.
#     # This involves products of velocity components and their spatial derivatives (gradients).
#     # For example, in 2D, the x-component of the convective term is u_x * d(u_x)/dx + u_y * d(u_x)/dy.
#     # This calculation is done point-wise in real space or using nodal values.
#     # Need to extract u_x_vec, u_y_vec from current_velocity_vector
#     # And d(ux)/dx_vec, d(ux)/dy_vec, d(uy)/dx_vec, d(uy)/dy_vec from velocity_gradient_x_n and velocity_gradient_y_n.
#     # convective_term_n_vector = calculate_convective_term_sem(current_velocity_vector, spatial_derivatives_n, grid_info) # Conceptual function call


#     # Outline the solution of the implicit system for the intermediate velocity field (u_star).
#     # This involves solving a linear system that includes the time derivative and implicit terms
#     # from the momentum equation (viscosity and hyper-viscosity terms), and the explicit terms.
#     # The system conceptually looks like:
#     # M * (u_star - u^n) / dt = Explicit_terms^n + Implicit_terms^star + Boundary_conditions
#     # Rearranging to solve for u_star:
#     # (M/dt - Implicit_Operator) * u_star = M/dt * u^n + Explicit_terms^n + Boundary_conditions
#     # Where M is the global mass matrix, Implicit_Operator represents the discretized implicit terms
#     # (e.g., -mu_fluid * nabla^2 - rho * nu_2 * nabla^4). The implicit operators (Laplacian, Bi-Laplacian)
#     # assembled in the previous SEM step are used here.
...
#     # Placeholder for formulating the right-hand side of the implicit system (RHS).
#     # This involves the previous time step's velocity, the explicit terms (convective), and boundary conditions.
#     # rhs_vector_implicit = formulate_implicit_rhs_vector(current_velocity_vector, convective_term_n_vector, global_operators, grid_info, time_step_size, rho, mu_fluid, nu_2) # Placeholder function

#     # Placeholder for solving the linear system: lhs_matrix_implicit * u_star_vector = rhs_vector_implicit
#     # This requires a linear solver (e.g., iterative solver like BiCGSTAB or GMRES for non-symmetric systems).
#     # u_star_vector, info = solve_linear_system(lhs_matrix_implicit, rhs_vector_implicit) # Conceptual solve function
#     u_star_vector = np.zeros_like(current_velocity_vector) # Placeholder output
...
#     # The RHS of the pressure-Poisson equation depends on the divergence of the intermediate velocity field (u_star)
#     # and other terms (viscous, hyper-viscous, convective).
#     # Calculate the divergence of the intermediate velocity field (u_star) using SEM.
#     # This requires applying the global divergence operator.
#     # u_star_derivatives = calculate_all_spatial_derivatives_sem_2d(u_star_vector, current_pressure_vector, global_operators, grid_info) # Call to implemented function on u_star
#     # divergence_u_star_vector = u_star_derivatives['velocity_divergence'] # Extract divergence


#     # Formulate the right-hand side vector for the pressure-Poisson equation.
#     # This function conceptually uses divergence_u_star_vector and potentially other derivative terms
#     # and the convective term (if the formulation requires it).
#     # rhs_pressure_poisson_vector = formulate_pressure_poisson_rhs(u_star_vector, convective_term_n_vector, time_step_size, rho, mu_fluid, nu_2, global_operators, grid_info) # Call to implemented function
#     rhs_pressure_poisson_vector = np.zeros(grid_info['num_global_dofs_scalar']) # Placeholder input for solve


#     # Solve the linear system A * p = rhs for the pressure field (p_new_vector).
#     # Matrix A is the assembled global Laplacian operator for pressure (PressureLaplacian_global),
#     # which is often the same as the velocity Laplacian matrix (K_global_laplacian) but
#     # potentially with different boundary conditions applied.
#     # pressure_field_new_vector = solve_pressure_poisson(rhs_pressure_poisson_vector, global_operators, grid_info) # Call to implemented function
#     pressure_field_new_vector = np.zeros(grid_info['num_global_dofs_scalar']) # Placeholder output


#     # --- Step 3: Project the Velocity Field ---

#     # Use the gradient of the new pressure field (p_new) to project u_star onto a divergence-free space.
#     # Calculate the gradient of the new pressure field using SEM.
#     # This requires applying the global gradient operator for pressure.
#     # pressure_derivatives_new = calculate_all_spatial_derivatives_sem_2d(current_velocity_vector, pressure_field_new_vector, global_operators, grid_info) # Call to implemented function on p_new
#     # pressure_gradient_new_vector = pressure_derivatives_new['pressure_gradient'] # Extract gradient (this is a vector field result)


#     # Project the intermediate velocity field using the pressure gradient.
#     # velocity_field_new_vector = project_velocity(u_star_vector, pressure_field_new_vector, time_step_size, rho, global_operators, grid_info) # Call to implemented function
#     velocity_field_new_vector = np.zeros_like(current_velocity_vector) # Placeholder output
...
#     # Update fields for the next time step.
#     # In a real implementation, this would involve updating the 'velocity_field' and 'pressure_field' arrays (field representations).
#     # For this conceptual loop, update the 'current_velocity_vector' and 'current_pressure_vector'.
#     current_velocity_vector = velocity_field_new_vector_bc # Update for next time step
#     current_pressure_vector = pressure_field_new_vector_bc # Update for next time step
...
#     # if (t_step + 1) % error_check_interval == 0 or t_step == num_time_steps - 1:
#         # print(f"Calculating conceptual error at time step {t_step + 1} (Time: {current_time:.4f})...")
#         # ... (Call error calculation function, comparing numerical vectors to analytical solution) ...


# print("-" * 30)
# print("Conceptual simulation loop structure outlined with integrated spatial derivative calls.")

print("Conceptual outline of the main simulation loop created with integrated spatial derivative calls.")
print("Includes conceptual calls to calculate_all_spatial_derivatives_sem_2d for velocity and pressure within the IMEX and projection method steps.")
print("Placeholder vectors are used to simulate data flow.")

"""## Implement IMEX time integration

**Subtask**: Develop code to implement the IMEX time integration scheme. This will involve splitting the terms in the equations into implicit and explicit parts and setting up the time-stepping loop.
"""
...
**Subtask**: Develop code to handle the pressure-velocity coupling using a projection method. This will involve formulating and solving the pressure-Poisson equation and projecting the velocity field to enforce incompressibility.
"""
...
**Subtask**: Define and implement the appropriate boundary conditions for the specific fluid dynamics problems being solved.
"""
...
**Subtask**: Create the main simulation loop that advances the solution in time, including calculating residuals, solving linear systems, and updating the solution variables.
"""
...
def assemble_mass_matrix_global_2d(num_elements_x, num_elements_y, sem_order, M_1d_x, M_1d_y, grid_info):
    ...
    Uses element-wise assembly.
    """
    print("\nAssembling Global 2D Mass matrix.")
    n_points_1d = sem_order + 1
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_2d = num_global_points_x * num_global_points_y
    ...
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            ...
            # Local elemental mass matrix contribution (scaled by Jacobian)
            # In SEM, the local mass matrix is diagonal (using GLL weights)
            jacobian_det = (element_size_x / 2.0) * (element_size_y / 2.0)
            local_mass_2d_contrib = np.outer(M_1d_y.diagonal(), M_1d_x.diagonal()) * jacobian_det
            local_mass_2d_contrib_flat = local_mass_2d_contrib.flatten()
...
def assemble_differentiation_matrices_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    ...
    Uses element-wise assembly based on the weak form.
    """
    print("\nAssembling Global 2D Differentiation matrices (Dx, Dy).")
    n_points_1d = sem_order + 1
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    num_global_dofs_2d = num_global_points_x * num_global_points_y

    # Initialize global sparse matrices (using LIL for efficient assembly)
    Dx_global_lil = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))
    Dy_global_lil = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))
    ...
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            ...
            # Elemental operators for differentiation in x and y (based on weak form integration by parts)
            # Local Dx operator (maps field to its derivative w.r.t x): M_1d_y kron (D_1d_x.T @ M_1d_x)
            # Local Dy operator (maps field to its derivative w.r.t y): (D_1d_y.T @ M_1d_y) kron M_1d_x

            # These are the operators that, when multiplied by the field vector and integrated
            # (which is handled by the global assembly process using the mass matrix implicitly
            # in the weak form formulation), give the derivative.
            # However, if we want an operator that directly maps nodal values to nodal derivative values,
            # we use the nodal differentiation matrices D_1d.
            # Global Dx (nodal): Identity_y kron D_x_global_1d_assembled
            # Global Dy (nodal): D_y_global_1d_assembled kron Identity_x

            # Let's assemble the nodal differentiation matrices as they are commonly used
            # for calculating derivatives at nodal points in strong form methods or for
            # constructing other operators.

            # Assemble global 1D Differentiation matrices first
            # Need to ensure D_1d_x and D_1d_y are based on nodes scaled to element size.
            # Or, scale the global assembled 1D matrices later.
            # Let's use the scaled 1D differentiation matrices directly in assembly.
            # D_1d_x_scaled = D_1d_x * (2.0 / element_size_x) # Scale from [-1, 1] to element size
            # D_1d_y_scaled = D_1d_y * (2.0 / element_size_y)

            # Instead of scaling D_1d, the Jacobian scaling is usually handled in the weak form integral
            # or when constructing the global operators from elemental matrices.
            # For nodal operators, let's assemble the global 1D D matrices first, then combine.
            # This requires the assemble_global_sem_operator function for D_1d.

            # Assuming assemble_global_sem_operator exists and works for D_1d.
            # We need the D_1d matrices based on the reference element [-1, 1].
            # D_1d_x, M_1d_x = compute_differentiation_matrix_1d(gll_nodes), compute_mass_matrix_1d(gll_weights) # Assuming same nodes/weights in x and y
            # D_1d_y, M_1d_y = D_1d_x, M_1d_x

            # Global 1D Differentiation matrices (assembled from reference D_1d)
            D_x_global_1d_assembled = assemble_global_sem_operator(D_1d_x, 'x', num_elements_x, sem_order)
            D_y_global_1d_assembled = assemble_global_sem_operator(D_1d_y, 'y', num_elements_y, sem_order)

            # Global 2D Differentiation matrices acting on flattened vector (assuming row-major flattening)
            # Dx operator applies D_x_global_1d_assembled independently to each row (along x)
            # This can be represented as a block diagonal matrix of D_x_global_1d_assembled
            num_global_points_x_total = num_elements_x * sem_order + 1
            num_global_points_y_total = num_elements_y * sem_order + 1
            num_global_dofs_scalar = num_global_points_x_total * num_global_points_y_total

            Dx_global_lil = block_diag([D_x_global_1d_assembled] * num_global_points_y).tolil()

            # Dy operator applies D_y_global_1d_assembled independently to each column (along y).
            # This requires assembling directly by looping through global nodes.
            Dy_global_lil = lil_matrix((num_global_dofs_scalar, num_global_dofs_scalar))

            for j_global in range(num_global_points_y_total):
                 for i_global in range(num_global_points_x_total):
                     global_row_idx = j_global * num_global_points_x_total + i_global
                     for l_global in range(num_global_points_y_total):
                         global_col_idx = l_global * num_global_points_x_total + i_global
                         # The derivative w.r.t y at (i_global, j_global) depends on values at (i_global, l_global) multiplied by D_y_global_1d_assembled[j_global, l_global]
                         Dy_global_lil[global_row_idx, global_col_idx] = D_y_global_1d_assembled[j_global, l_global]

    print("Global 2D Differentiation matrices (Dx_nodal, Dy_nodal) assembled.")
    # Convert to CSR for efficient matrix-vector products
    return Dx_global_lil.tocsr(), Dy_global_lil.tocsr()
...
def assemble_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    """
    Assembles the global 2D Laplacian matrix (nabla^2).
    Assembles the Stiffness matrix K based on the weak form: K = K_x + K_y.
    K_x_elemental = D_1d_x.T @ M_1d_x @ D_1d_x
    K_y_elemental = D_1d_y.T @ M_1d_y @ D_1d_y
    Global K_x += K_x_elemental kron M_1d_y * Jacobian_y_scaling
    Global K_y += M_1d_x kron K_y_elemental * Jacobian_x_scaling
    Laplacian operator in strong form is M_global_inv * (-K_global).
    This function assembles K_global.
    """
    print("\nAssembling Global 2D Laplacian (Stiffness) Matrix (K).")
    ...
    K_global_lil = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))

    element_size_x = grid_info['domain_size_x'] / num_elements_x
    element_size_y = grid_info['domain_size_y'] / num_elements_y
    # Jacobian scaling for the integral transformation: physical_element = mapping(reference_element)
    # For uniform rectangular elements, dx dy = |det(J)| dxi deta, where |det(J)| = (element_size_x/2) * (element_size_y/2)
    jacobian_det = (element_size_x / 2.0) * (element_size_y / 2.0)
    ...
    for ey in range(num_elements_y):
        for ex in range(num_elements_x):
            ...
            # Elemental stiffness matrices in 1D (from reference element)
            K_1d_x_ref = D_1d_x.T @ M_1d_x @ D_1d_x
            K_1d_y_ref = D_1d_y.T @ M_1d_y @ D_1d_y

            # Elemental mass matrices in 1D (from reference element) - these are diagonal
            M_1d_x_ref = M_1d_x
            M_1d_y_ref = M_1d_y

            # Elemental stiffness matrices in 2D for each component of Laplacian (based on weak form)
            # Integral of (dphi/dx * dpsi/dx) dOmega over element
            # = int (dphi/dxi * dxi/dx) * (dpsi/dxi * dxi/dx) |det(J)| dxi deta
            # = int (dphi/dxi * dpsi/dxi) (dxi/dx)^2 |det(J)| dxi deta
            # (dxi/dx) = 2/element_size_x. (dxi/dx)^2 = (2/element_size_x)^2.
            # Need to integrate over deta as well, with basis in eta.
            # Elemental Kxx contribution: (D_1d_x.T @ M_1d_x @ D_1d_x) kron M_1d_y * (2/element_size_x)^2 * jacobian_det (No, this is not quite right)

            # Correct elemental stiffness assembly for tensor product elements:
            # K_elemental = Kxx_elemental + Kyy_elemental
            # Kxx_elemental = (D_1d_x.T @ M_1d_x @ D_1d_x) kron M_1d_y  * (element_size_y/2) * (2/element_size_x)^2 * (element_size_x/2) # This gets messy
            # Let's use the simpler form: K_elemental = (D_1d_x.T M_1d_x D_1d_x) kron M_1d_y + M_1d_x kron (D_1d_y.T M_1d_y D_1d_y)
            # And apply Jacobian scaling to the total elemental matrix.

            Kxx_elemental_2d = np.kron(K_1d_x_ref, M_1d_y_ref)
            Kyy_elemental_2d = np.kron(M_1d_x_ref, K_1d_y_ref)

            K_elemental_2d = Kxx_elemental_2d + Kyy_elemental_2d # Elemental Stiffness Matrix in 2D (reference)

            # Scale by element Jacobian determinant for the integral transformation
            K_elemental_scaled = K_elemental_2d * jacobian_det
            ...
            for local_row_flat in range(n_points_1d * n_points_1d):
                for local_col_flat in range(n_points_1d * n_points_1d):
                     global_row = global_indices[local_row_flat]
                     global_col = global_indices[local_col_flat]
                     K_global_lil[global_row, global_col] += K_elemental_scaled[local_row_flat, local_col_flat]


    print("Global 2D Laplacian (Stiffness) matrix assembled.")
    return K_global_lil.tocsr() # Convert to CSR


# Implement function to assemble the global 2D Bi-Laplacian matrix (nabla^4)
# This is more complex and involves assembling a dedicated Bi-Laplacian stiffness matrix
# based on the weak form: int (nabla^2 v) (nabla^2 u) dOmega.
# The elemental Bi-Laplacian stiffness matrix involves terms like
# (D_1d_x.T M_1d_x D_1d_x) kron (D_1d_y.T M_1d_y D_1d_y) + 2 * (D_1d_x.T M_1d_x D_1d_x) kron (D_1d_y.T M_1d_y D_1d_y) + ...
# It's the integral of (v_xx + v_yy)(u_xx + u_yy) dOmega
# = int (v_xx u_xx + v_xx u_yy + v_yy u_xx + v_yy u_yy) dOmega
# Each term needs elemental assembly. Example: int v_xx u_xx dOmega_e = int (d^2v/dxi^2 * (dxi/dx)^2) (d^2u/dxi^2 * (dxi/dx)^2) |det(J)| dxi deta
# = int (d^2v/dxi^2 d^2u/dxi^2) (dxi/dx)^4 |det(J)| dxi deta
# = int (d^2v/dxi^2 d^2u/dxi^2) (2/element_size_x)^4 * (element_size_x/2)*(element_size_y/2) dxi deta
# = int (d^2v/dxi^2 d^2u/dxi^2) * (16/element_size_x^4) * (element_size_x * element_size_y / 4) dxi deta
# = int (d^2v/dxi^2 d^2u/dxi^2) * (4 * element_size_y / element_size_x^3) dxi deta
# This shows the complexity of Jacobian scaling for higher-order derivatives.

def assemble_bi_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, M_1d_x, D_1d_y, M_1d_y, grid_info):
    """
    Assembles the global 2D Bi-Laplacian matrix (nabla^4).
    Assembles the Bi-Laplacian Stiffness matrix K_bi based on the weak form: int (nabla^2 v) (nabla^2 u) dOmega.
    This implementation focuses on the elemental assembly of the terms.
    """
    print("\nAssembling Global 2D Bi-Laplacian (Stiffness) Matrix (K_bi).")
    ...
    K_bi_global_lil = lil_matrix((num_global_dofs_2d, num_global_dofs_2d))

    element_size_x = grid_info['domain_size_x'] / num_elements_x
    element_size_y = grid_info['domain_size_y'] / num_elements_y
    jacobian_det = (element_size_x / 2.0) * (element_size_y / 2.0)

    # Elemental operators in 1D (from reference element)
    D_1d_x_ref = D_1d_x
    M_1d_x_ref = M_1d_x
    D_1d_y_ref = D_1d_y
    M_1d_y_ref = M_1d_y

    # Elemental stiffness matrices for 1D second derivatives
    K_1d_x_ref = D_1d_x_ref.T @ M_1d_x_ref @ D_1d_x_ref
    K_1d_y_ref = D_1d_y_ref.T @ M_1d_y_ref @ D_1d_y_ref

    # Elemental stiffness matrices for 1D fourth derivatives (conceptual, based on weak form)
    # int (v'') (u'') dxi = int (v' * u''') dxi = int (v * u'''') dxi (Integration by parts)
    # In SEM, (D^2)^T M (D^2) or K.T M_inv K (approx)
    # Let's use K.T M_inv K for elemental approximation, or D.T M D.T M D.T M D ... this is too complex
    # A common approach for Bi-Laplacian in SEM is to use the weak form int (nabla^2 v) (nabla^2 u) dOmega.
    # The elemental contribution is int_Omega_e (v_xx + v_yy)(u_xx + u_yy) dOmega_e
    # = int_Omega_e (v_xx u_xx + v_xx u_yy + v_yy u_xx + v_yy u_yy) dOmega_e

    # Elemental contributions to the Bi-Laplacian stiffness matrix (scaled by appropriate Jacobians)
    # Term 1: int v_xx u_xx dOmega_e -> requires D_x.T M_x D_x in x and M_y in y, scaled by (2/element_size_x)^2 * jacobian_det
    # This scales to (D_1d_x.T M_1d_x D_1d_x) kron M_1d_y * (element_size_y / 2) * (2/element_size_x)^2 * (element_size_x/2)  -- Still looks complicated.

    # Let's use a simpler approach based on applying the Laplacian operator twice using elemental matrices.
    # nabla^4 u = nabla^2 (nabla^2 u)
    # Elemental matrix for nabla^2: K_elemental_2d = Kxx_elemental_2d + Kyy_elemental_2d (from Laplacian assembly)

    # This approach is more about applying the operator in a matrix-vector product, not assembling K_bi.
    # To assemble K_bi from elemental contributions based on int (nabla^2 v)(nabla^2 u) dOmega:
    # Terms involve combinations of second derivative operators in x and y.
    # Elemental operator for d^2/dx^2: D_1d_x.T M_1d_x D_1d_x kron M_1d_y
    # Elemental operator for d^2/dy^2: M_1d_x kron D_1d_y.T M_1d_y D_1d_y

    # Elemental K_bi contribution involves tensor products of these second derivative operators and M_1d terms,
    # scaled by appropriate Jacobian factors. This is quite involved and prone to errors without a
    # clear variational formulation implementation.

    # As this is a conceptual implementation, we will use a simplified placeholder.
    # A correct implementation would involve elemental assembly of the Bi-Laplacian weak form.

    print("Bi-Laplacian assembly is complex and this is a placeholder.")
...
def assemble_gradient_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, D_1d_y):
    """
    Assembles the global 2D Gradient operator (nabla), mapping from scalar to vector field DOFs.
    Returns a block matrix [Gx_global, Gy_global]^T where Gx/Gy map scalar field DOFs to scalar field component derivative DOFs.
    Uses the nodal differentiation matrices (D_global).
    """
    print("\nAssembling Global 2D Gradient Operator.")
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    ...
    # Assemble global 1D Differentiation matrices (nodal)
    D_x_global_1d_assembled = assemble_global_sem_operator(D_1d_x, 'x', num_elements_x, sem_order)
    D_y_global_1d_assembled = assemble_global_sem_operator(D_1d_y, 'y', num_elements_y, sem_order)

    # Gx operator: Applies D_x_global_1d_assembled independently to each row
    Gx_global = block_diag([D_x_global_1d_assembled] * num_global_points_y)

    # Gy operator: Applies D_y_global_1d_assembled independently to each column.
    # For row-major flattening, this requires assembling directly by looping through global nodes.
    Gy_global = lil_matrix((num_global_dofs_scalar, num_global_dofs_scalar))
    ...
    # This operator maps a scalar pressure vector (size num_global_dofs_scalar)
    # to a concatenated velocity gradient vector [dp/dx_vec, dp/dy_vec] (size 2 * num_global_dofs_scalar).
    # Gradient_global = csr_matrix(block_diag([Gx_global, Gy_global])) # This is if we return the full gradient operator


    print("Global 2D Gradient operator (components Gx_nodal, Gy_nodal) assembled.")
    # Return Gx_global and Gy_global separately as they map scalar to scalar component derivative.
    return Gx_global.tocsr(), Gy_global.tocsr()
...
def assemble_divergence_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d_x, D_1d_y):
    ...
    Assembles the global 2D Divergence operator (nabla .), mapping from vector to scalar field DOFs.
    Returns a block matrix [Div_x_global Div_y_global] where Div_x/Div_y map scalar field components to scalar field.
    Uses the transpose of the nodal differentiation matrices (D_global.T).
    """
    print("\nAssembling Global 2D Divergence Operator.")
    num_global_points_x = num_elements_x * sem_order + 1
    num_global_points_y = num_elements_y * sem_order + 1
    ...
    print("Global 2D Divergence operator (components Div_x_nodal, Div_y_nodal, and combined Divergence_nodal) assembled.")
    # Return Div_x_global, Div_y_global, and the combined Divergence_global
    return Div_x_global.tocsr(), Div_y_global.tocsr(), Divergence_global.tocsr()


# Example of how to use these assembly functions (requires grid_info, D_1d, M_1d)
# Assuming grid_info is available from the test case setup.
# Assuming D_1d and M_1d are available from the 1D SEM implementation.

# num_elements_x_val = grid_info['num_elements_x']
# num_elements_y_val = grid_info['num_elements_y']
# sem_order_val = grid_info['sem_order']

# global_M = assemble_mass_matrix_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, M_1d, M_1d, grid_info)
# print(f"\nAssembled Global Mass Matrix shape: {global_M.shape}")

# global_Dx_nodal, global_Dy_nodal = assemble_differentiation_matrices_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, D_1d, M_1d, D_1d, M_1d, grid_info)
# print(f"\nAssembled Global Nodal Dx Matrix shape: {global_Dx_nodal.shape}")
# print(f"Assembled Global Nodal Dy Matrix shape: {global_Dy_nodal.shape}")

# global_K_laplacian = assemble_laplacian_matrix_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, D_1d, M_1d, D_1d, M_1d, grid_info)
# print(f"\nAssembled Global Laplacian Stiffness Matrix shape: {global_K_laplacian.shape}")

# global_K_bilaplacian = assemble_bi_laplacian_matrix_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, D_1d, M_1d, D_1d, M_1d, grid_info) # Placeholder
# print(f"\nAssembled Global Bi-Laplacian Stiffness Matrix shape: {global_K_bilaplacian.shape} (Placeholder)")

# global_Gx_nodal, global_Gy_nodal = assemble_gradient_operator_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, D_1d, D_1d)
# print(f"\nAssembled Global Nodal Gx Operator shape: {global_Gx_nodal.shape}")
# print(f"Assembled Global Nodal Gy Operator shape: {global_Gy_nodal.shape}")

# global_Div_x_nodal, global_Div_y_nodal, global_Divergence_nodal = assemble_divergence_operator_global_2d(num_elements_x_val, num_elements_y_val, sem_order_val, D_1d, D_1d)
# print(f"\nAssembled Global Nodal Div_x Operator shape: {global_Div_x_nodal.shape}")
# print(f"Assembled Global Nodal Div_y Operator shape: {global_Div_y_nodal.shape}")
# print(f"Assembled Global Nodal Divergence Operator shape: {global_Divergence_nodal.shape}")


print("\nGlobal SEM assembly functions implemented.")
print("Includes functions for assembling Mass, Differentiation (nodal Dx, Dy), Laplacian (Stiffness), Bi-Laplacian (Stiffness - placeholder), Gradient (nodal Gx, Gy), and Divergence (nodal Divx, Divy, and combined Divergence) operators.")
print("Uses element-wise assembly for Mass and Laplacian Stiffness.")
print("Uses combinations of assembled 1D nodal differentiation matrices for nodal Differentiation, Gradient, and Divergence.")
print("Note: Bi-Laplacian assembly is a placeholder and needs detailed implementation based on the weak form.")
...
"""## Implement field reshaping functions

**Subtask**: Implement functions to convert between the grid representation of fields and the flattened vector representation used by global SEM operators.
"""

import numpy as np

# Assuming grid_info is available, containing grid dimensions and mapping information.
# grid_info = {
#     'num_global_points_x': None,
#     'num_global_points_y': None,
#     'num_global_points_z': None, # For 3D
#     'num_global_dofs_scalar': None, # Total scalar DOFs (nx * ny * nz)
#     'num_global_dofs_velocity': None, # Total velocity DOFs (nx * ny * nz * ndim)
#     'ndim': None, # Number of spatial dimensions (2 or 3)
#     # Add mapping info if needed (e.g., from flattened vector index to grid index)
# }

print("Implementing field reshaping functions.")

def reshape_field_to_vector(field, grid_info):
    """
    Reshapes a field from its grid representation to a flattened vector.

    Args:
        field (np.ndarray): The input field in grid representation (e.g., (nx, ny) for scalar 2D,
                            (nx, ny, 2) for vector 2D, (nx, ny, nz) for scalar 3D,
                            (nx, ny, nz, 3) for vector 3D).
        grid_info (dict): Information about the grid, including 'ndim'.

    Returns:
        np.ndarray: The flattened vector representation of the field.
                    For a vector field, assumes concatenation of components
                    (e.g., [u_x_vec, u_y_vec, u_z_vec]).
    """
    print(f"\nReshaping field with shape {field.shape} to vector.")

    ndim = grid_info.get('ndim', field.ndim if field.ndim <= 3 else field.ndim - 1) # Infer ndim if not provided

    if ndim == 2:
        if field.ndim == 2: # Scalar field (e.g., pressure)
            return field.flatten()
        elif field.ndim == 3 and field.shape[-1] == 2: # Vector field (e.g., velocity [ux, uy])
            # Flatten each component and concatenate
            u_x_flat = field[:, :, 0].flatten()
            u_y_flat = field[:, :, 1].flatten()
            return np.concatenate([u_x_flat, u_y_flat])
        else:
            print(f"Warning: Unexpected field shape {field.shape} for 2D. Attempting generic flatten.")
            return field.flatten()

    elif ndim == 3:
        if field.ndim == 3: # Scalar field (e.g., pressure)
             return field.flatten()
        elif field.ndim == 4 and field.shape[-1] == 3: # Vector field (e.g., velocity [ux, uy, uz])
             # Flatten each component and concatenate
             u_x_flat = field[:, :, :, 0].flatten()
             u_y_flat = field[:, :, :, 1].flatten()
             u_z_flat = field[:, :, :, 2].flatten()
             return np.concatenate([u_x_flat, u_y_flat, u_z_flat])
        else:
             print(f"Warning: Unexpected field shape {field.shape} for 3D. Attempting generic flatten.")
             return field.flatten()
    else:
        print(f"Error: Unsupported number of dimensions: {ndim}. Cannot reshape.")
        return field.flatten() # Fallback to generic flatten


def reshape_vector_to_field(vector, grid_info, field_type='scalar'):
    """
    Reshapes a flattened vector back to its grid representation.

    Args:
        vector (np.ndarray): The flattened vector representation of the field.
        grid_info (dict): Information about the grid, including 'num_global_points_x',
                          'num_global_points_y', 'num_global_points_z', and 'ndim'.
        field_type (str): The type of field ('scalar' or 'vector'). Determines the output shape.

    Returns:
        np.ndarray: The field in its grid representation.
    """
    print(f"\nReshaping vector with size {vector.shape[0]} to {field_type} field.")

    nx = grid_info.get('num_global_points_x')
    ny = grid_info.get('num_global_points_y')
    nz = grid_info.get('num_global_points_z')
    ndim = grid_info.get('ndim')

    if nx is None or ny is None or ndim is None:
        print("Error: Missing grid dimensions (num_global_points_x, num_global_points_y, ndim) in grid_info.")
        # Return original vector if grid info is insufficient
        return vector
    ...
    if ndim == 2:
        if field_type == 'scalar':
            ...
            if vector.shape[0] != expected_size:
                 print(f"Warning: Vector size {vector.shape[0]} does not match expected scalar field size {expected_size} for 2D.")
            return vector.reshape((ny, nx)).T # Assuming (ny, nx) reshape followed by transpose for (nx, ny)
                                             # Need to be careful with ordering (row-major vs column-major flattening)
                                             # If row-major flattening was used: return vector.reshape((ny, nx))

        elif field_type == 'vector':
            expected_size = nx * ny * 2
            if vector.shape[0] != expected_size:
                print(f"Warning: Vector size {vector.shape[0]} does not match expected vector field size {expected_size} for 2D.")

            # Assuming concatenated components [u_x_vec, u_y_vec]
            num_scalar_dofs = nx * ny
            if vector.shape[0] >= num_scalar_dofs * 2:
                 u_x_flat = vector[:num_scalar_dofs]
                 u_y_flat = vector[num_scalar_dofs:num_scalar_dofs*2]

                 u_x_field = u_x_flat.reshape((ny, nx)).T # Reshape and transpose to (nx, ny)
                 u_y_field = u_y_flat.reshape((ny, nx)).T

                 return np.stack([u_x_field, u_y_field], axis=-1) # Stack to form (nx, ny, 2)
            else:
                 print(f"Error: Vector size {vector.shape[0]} is too small for 2D vector field.")
                 return vector # Return original vector as fallback

        else:
            print(f"Error: Unknown field type '{field_type}' for reshaping in 2D.")
            return vector # Return original vector

    elif ndim == 3:
        if nz is None:
            print("Error: Missing num_global_points_z for 3D reshaping in grid_info.")
            return vector

        if field_type == 'scalar':
            expected_size = nx * ny * nz
            if vector.shape[0] != expected_size:
                print(f"Warning: Vector size {vector.shape[0]} does not match expected scalar field size {expected_size} for 3D.")
            return vector.reshape((nz, ny, nx)).transpose(2, 1, 0) # Reshape and transpose to (nx, ny, nz)
                                                                 # Need to be careful with ordering

        elif field_type == 'vector':
             expected_size = nx * ny * nz * 3
             if vector.shape[0] != expected_size:
                  print(f"Warning: Vector size {vector.shape[0]} does not match expected vector field size {expected_size} for 3D.")

             # Assuming concatenated components [u_x_vec, u_y_vec, u_z_vec]
             num_scalar_dofs = nx * ny * nz
             if vector.shape[0] >= num_scalar_dofs * 3:
                  u_x_flat = vector[:num_scalar_dofs]
                  u_y_flat = vector[num_scalar_dofs:num_scalar_dofs*2]
                  u_z_flat = vector[num_scalar_dofs*2:num_scalar_dofs*3]

                  u_x_field = u_x_flat.reshape((nz, ny, nx)).transpose(2, 1, 0) # Reshape and transpose to (nx, ny, nz)
                  u_y_field = u_y_flat.reshape((nz, ny, nx)).transpose(2, 1, 0)
                  u_z_field = u_z_flat.reshape((nz, ny, nx)).transpose(2, 1, 0)

                  return np.stack([u_x_field, u_y_field, u_z_field], axis=-1) # Stack to form (nx, ny, nz, 3)
             else:
                  print(f"Error: Vector size {vector.shape[0]} is too small for 3D vector field.")
                  return vector # Return original vector as fallback
        else:
            print(f"Error: Unknown field type '{field_type}' for reshaping in 3D.")
            return vector # Return original vector
    else:
        print(f"Error: Unsupported number of dimensions: {ndim}. Cannot reshape.")
        return vector # Return original vector
...
print("\nField reshaping functions implemented.")
print("Includes functions to reshape between grid representation and flattened vector representation for scalar and vector fields in 2D and 3D.")
print("Note: Assumes concatenated vector components for vector fields and requires correct grid information in grid_info.")

"""## Integrate SEM into solver structure

**Subtask**: Integrate the implemented SEM components (global assembly, spatial derivative calculation, and field reshaping) into the main simulation loop to compute the necessary spatial derivative terms at each time step.
"""

# Integrate SEM components into the main simulation loop structure.
...
# Conceptual Grid Information and Initial Fields (already defined in a previous cell)
# grid_info = ...
# velocity_field = ... # Assuming field representation (nx, ny, ndim)
# pressure_field = ... # Assuming field representation (nx, ny)

# Placeholder for assembled global operators (needs to be populated before the loop)
# global_operators = {
#     'M_global': None, # Global Mass matrix (CSR)
#     'K_global_laplacian': None, # Global Laplacian Stiffness matrix (CSR)
#     'K_global_bilaplacian': None, # Global Bi-Laplacian Stiffness matrix (CSR) - placeholder assembly
...
#     'Div_x_global': None, # Global Divergence operator component d/dx (CSR)
#     'Div_y_global': None, # Global Divergence operator component d/dy (CSR)
#     # Add Pressure Laplacian if different from velocity Laplacian due to BCs
#     # 'PressureLaplacian_global': None,
# }

# Placeholder grid_info with necessary keys for spatial derivative and reshape functions
# This should be populated by the grid creation function before the loop.
grid_info = {
    'num_global_points_x': 17, # Example value based on previous test case setup
    'num_global_points_y': 17, # Example value
    'ndim': 2, # Example value for 2D
    'num_global_dofs_scalar': 17 * 17, # Total scalar DOFs (nx * ny)
    'num_global_dofs_velocity': 17 * 17 * 2, # Total velocity DOFs (nx * ny * ndim)
    'domain_size_x': 2 * np.pi, # Example value
    'domain_size_y': 2 * np.pi, # Example value
    'global_node_coords': None # Placeholder - needs to be populated
    # Add other necessary grid info like boundary node indices, mapping info, etc.
}

# Re-define conceptual simulation parameters for the loop structure
num_time_steps = 200
time_step_size = 0.001
rho = 1.0
mu_fluid = 0.01
...
print("Integrating SEM components into the main simulation loop structure.")

# --- Conceptual Setup Before the Loop ---

# 1. Create Grid and Initialize Fields (Conceptual - requires actual implementation)
# global_node_coords, grid_info = create_uniform_sem_grid_2d(num_elements_x, num_elements_y, domain_size_x, domain_size_y, sem_order)
# initial_velocity_field, initial_pressure_field = initialize_taylor_green_vortex_2d(global_node_coords, rho, mu_fluid)

# Initialize current fields (using placeholders with correct conceptual shapes)
# In a real implementation, these would be initialized from initial_velocity_field and initial_pressure_field
num_global_points_x = grid_info['num_global_points_x']
num_global_points_y = grid_info['num_global_points_y']
field_dimension = (num_global_points_x, num_global_points_y)
current_velocity_field = np.zeros((*field_dimension, grid_info['ndim'])) # Placeholder field
current_pressure_field = np.zeros(field_dimension) # Placeholder field

# Populate grid_info['global_node_coords'] for conceptual error calculation
x_coords_1d = np.linspace(0, grid_info['domain_size_x'], num_global_points_x)
y_coords_1d = np.linspace(0, grid_info['domain_size_y'], num_global_points_y)
global_node_coords_x_2d, global_node_coords_y_2d = np.meshgrid(x_coords_1d, y_coords_1d, indexing='ij')
grid_info['global_node_coords'] = np.stack([global_node_coords_x_2d, global_node_coords_y_2d], axis=-1)


# 2. Assemble Global Operators (Conceptual - requires actual implementation)
# Requires D_1d and M_1d from 1D SEM implementation.
# global_operators['M_global'] = assemble_mass_matrix_global_2d(num_elements_x, num_elements_y, sem_order, M_1d, M_1d, grid_info)
# global_operators['K_global_laplacian'] = assemble_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d, M_1d, D_1d, M_1d, grid_info)
# global_operators['K_global_bilaplacian'] = assemble_bi_laplacian_matrix_global_2d(num_elements_x, num_elements_y, sem_order, D_1d, M_1d, D_1d, M_1d, grid_info) # Placeholder
# global_operators['Gx_global'], global_operators['Gy_global'] = assemble_gradient_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d, D_1d)
# global_operators['Div_x_global'], global_operators['Div_y_global'], global_operators['Divergence_global'] = assemble_divergence_operator_global_2d(num_elements_x, num_elements_y, sem_order, D_1d, D_1d)
# Assuming Pressure Laplacian is the same as velocity Laplacian for now for simplicity in this outline
# global_operators['PressureLaplacian_global'] = global_operators['K_global_laplacian']
# Assuming Pressure Gradient is the same as velocity Gradient operator components
# global_operators['PressureGradient_global'] = global_operators['Gradient_global'] # If combined operator is used
# Or, use Gx_global and Gy_global for pressure gradient calculation

# Initialize global_operators dictionary (empty for now, will be populated by assembly functions)
global_operators = {}
...
print("\nConceptual Setup Before the Loop Complete.")
print("Starting conceptual simulation loop.")
...
for t_step in range(num_time_steps):
    ...
    # Convert current fields from grid representation to flattened vectors
    current_velocity_vector = reshape_field_to_vector(current_velocity_field, grid_info)
    current_pressure_vector = reshape_field_to_vector(current_pressure_field, grid_info)
    ...
    # Extract necessary derivatives for the IMEX step
    velocity_gradient_x_n = spatial_derivatives_n['velocity_gradient_x'] # Gradient of u_x (vector [d(ux)/dx, d(ux)/dy])
    velocity_gradient_y_n = spatial_derivatives_n['velocity_gradient_y'] # Gradient of u_y (vector [d(uy)/dx, d(uy)/dy])
    velocity_laplacian_x_n = spatial_derivatives_n['velocity_laplacian_x'] # Laplacian of u_x (scalar)
    velocity_laplacian_y_n = spatial_derivatives_n['velocity_laplacian_y'] # Laplacian of u_y (scalar)
    velocity_bi_laplacian_x_n = spatial_derivatives_n['velocity_bi_laplacian_x'] # Bi-Laplacian of u_x (scalar)
    velocity_bi_laplacian_y_n = spatial_derivatives_n['velocity_bi_laplacian_y'] # Bi-Laplacian of u_y (scalar)


    # Implement the calculation of the explicit convective term (u.grad)u at time t^n.
    # This involves products of velocity components and their spatial derivatives (gradients).
    # For example, in 2D, the x-component of the convective term is u_x * d(u_x)/dx + u_y * d(u_x)/dy.
    # This calculation is done point-wise using the values at the GLL nodes (represented by the vectors).
    # Need to extract u_x_vec, u_y_vec from current_velocity_vector
    num_scalar_dofs = grid_info['num_global_dofs_scalar']
    u_x_vec = current_velocity_vector[:num_scalar_dofs]
    u_y_vec = current_velocity_vector[num_scalar_dofs:]

    # Extract derivative components from gradient vectors (each gradient is [d/dx, d/dy] applied to the component)
    dux_dx_vec = velocity_gradient_x_n[:num_scalar_dofs]
    dux_dy_vec = velocity_gradient_x_n[num_scalar_dofs:]
    duy_dx_vec = velocity_gradient_y_n[:num_scalar_dofs]
    duy_dy_vec = velocity_gradient_y_n[num_scalar_dofs:]

    # Calculate convective term components point-wise
    convective_term_x_vec = u_x_vec * dux_dx_vec + u_y_vec * dux_dy_vec
    convective_term_y_vec = u_x_vec * duy_dx_vec + u_y_vec * duy_dy_vec

    # Combine convective term components into a single vector
    convective_term_n_vector = np.concatenate([convective_term_x_vec, convective_term_y_vec])

    # print("Explicit convective term (u.grad)u calculated.")
    ...
    # Placeholder for assembling the matrix for the implicit system (LHS).
    # This matrix depends on the global mass matrix and the assembled global operators for implicit terms.
    # lhs_matrix_implicit = assemble_implicit_lhs_matrix(global_operators, grid_info, time_step_size, rho, mu_fluid, nu_2) # Placeholder function call

    # Placeholder for formulating the right-hand side of the implicit system (RHS).
    # This involves the previous time step's velocity, the explicit terms (convective), and boundary conditions.
    # rhs_vector_implicit = formulate_implicit_rhs_vector(current_velocity_vector, convective_term_n_vector, global_operators, grid_info, time_step_size, rho, mu_fluid, nu_2) # Placeholder function call

    # Placeholder for solving the linear system: lhs_matrix_implicit * u_star_vector = rhs_vector_implicit
    # This requires a linear solver (e.g., iterative solver like BiCGSTAB or GMRES for non-symmetric systems).
    # u_star_vector, info = solve_linear_system(lhs_matrix_implicit, rhs_vector_implicit) # Conceptual solve function
    # Using a placeholder zero vector for u_star_vector for now.
    u_star_vector = np.zeros_like(current_velocity_vector)
    ...
    # and the convective term (if the formulation requires it).
    # rhs_pressure_poisson_vector = formulate_pressure_poisson_rhs(u_star_vector, convective_term_n_vector, time_step_size, rho, mu_fluid, nu_2, global_operators, grid_info) # Call to implemented function
    # Using a placeholder zero vector for rhs_pressure_poisson_vector for now.
    rhs_pressure_poisson_vector = np.zeros(grid_info['num_global_dofs_scalar'])
    ...
    # potentially with different boundary conditions applied.
    # pressure_field_new_vector = solve_pressure_poisson(rhs_pressure_poisson_vector, global_operators, grid_info) # Call to implemented function
    # Using a placeholder zero vector for pressure_field_new_vector for now.
    pressure_field_new_vector = np.zeros(grid_info['num_global_dofs_scalar'])
    ...
    # Project the intermediate velocity field using the pressure gradient.
    # velocity_field_new_vector = project_velocity(u_star_vector, pressure_field_new_vector, time_step_size, rho, global_operators, grid_info) # Call to implemented function
    # Using a placeholder zero vector for velocity_field_new_vector for now.
    velocity_field_new_vector = np.zeros_like(u_star_vector) # Placeholder output
    ...
    # --- Step 5: Update Fields and Advance Time ---
    # Update fields for the next time step.
    # Convert the updated velocity and pressure vectors back to field representation.
    current_velocity_field = reshape_vector_to_field(velocity_field_new_vector_bc, grid_info, field_type='vector')
    current_pressure_field = reshape_vector_to_field(pressure_field_new_vector_bc, grid_info, field_type='scalar')

    # Advance simulation time is already done at the top of the loop
    ...
    # if (t_step + 1) % save_interval == 0 or t_step == num_time_steps - 1:
        # print(f"Saving/Visualizing results at time step {t_step + 1}...")
        # ... (Call saving/visualization functions, potentially using the field representations) ...
    ...
    if (t_step + 1) % error_check_interval == 0 or t_step == num_time_steps - 1:
        ...
        # Reshape analytical fields to vectors for comparison with numerical vectors
        # In a real implementation, ensure consistency in vector flattening.
        analytical_velocity_vector = reshape_field_to_vector(analytical_velocity_field, grid_info)
        analytical_pressure_vector = reshape_field_to_vector(analytical_pressure_field, grid_info)
        ...
        # Compute L2 norm of the error
        velocity_error_l2 = np.linalg.norm(velocity_error_vector) / np.sqrt(grid_info['num_global_dofs_velocity']) # Normalize by sqrt(number of DOFs)
        pressure_error_l2 = np.linalg.norm(pressure_error_vector) / np.sqrt(grid_info['num_global_dofs_scalar']) # Normalize by sqrt(number of DOFs)
...
print("-" * 30)
print("Conceptual simulation loop finished with integrated SEM component calls.")

# Acknowledgment: The actual implementation requires replacing the commented-out
# conceptual calls with fully implemented functions for each solver component,
# ensuring correct data flow and handling of boundary conditions.
# The error calculation here is conceptual as the numerical solution is a placeholder.