...
def solve_poisson_ms(nx: int, ny: int, tol: float = 1e-8, max_iter: int = 50_000, omega: float = 1.0, progress: bool = False, progress_every: int = 500) -> PoissonResult:
    ...
    # Re-impose Dirichlet BCs after each Jacobi iteration (on the new solution buffer)
    u[0, :] = u_exact[0, :]
    u[-1, :] = u_exact[-1, :]
    u[:, 0] = u_exact[:, 0]
    u[:, -1] = u_exact[:, -1]
...
def run_suite(grids: List[int] | None = None, tol: float = 1e-8, omega: float = 1.0, progress: bool = False, run_logger=None) -> Dict[str, List[Dict[str, float]]]:
    ...
    orders: list[float] = []  # initialize early
    # Collect L2 values from structured result objects
    try:
        l2_values = [getattr(r, 'l2_error', None) for r in results]
        l2_values = [v for v in l2_values if isinstance(v, (int, float))]
        if l2_values:
            for i in range(1, len(l2_values)):
                if l2_values[i] > 0 and l2_values[i-1] > 0:
                    orders.append((math.log10(l2_values[i-1]) - math.log10(l2_values[i])))
        reported_order = orders[-1] if orders else 0.0
        min_l2 = min(l2_values) if l2_values else float('inf')
    except Exception:
        reported_order = 0.0
        min_l2 = float('inf')
    ...
    # When emitting summary ensure safe use of orders list
    min_order = min(orders) if orders else None
    max_order = max(orders) if orders else None
    print('Min L2:', min_l2, 'Last observed order drop:', reported_order, 'min_order:', min_order, 'max_order:', max_order)
    if run_logger:
...
if __name__ == "__main__":
    ...
    if args.ci_gate:
        import sys
        orders_ci = out["pairwise_L2_order"]
        orders_ci_raw = orders_ci
        # Normalize any non-float entries (dicts) to floats if present
        numeric_orders = []
        for o in orders_ci_raw:
            if isinstance(o, (int,float)):
                numeric_orders.append(float(o))
            elif isinstance(o, dict):
                val = o.get('order') if 'order' in o else None
                if isinstance(val, (int,float)):
                    numeric_orders.append(float(val))
        if not numeric_orders:
            print("CI GATE FAIL: no numeric orders parsed (insufficient grids)")
            sys.exit(1)
        failing = [i for i,o in enumerate(numeric_orders) if o < args.order_threshold]
        if failing:
            print(f"CI GATE FAIL: pair indices {failing} below threshold {args.order_threshold} (orders={numeric_orders})")
            sys.exit(1)
        ...
        if finest["linf_residual"] > args.tol:
            ...
            sys.exit(1)
        print(f"CI GATE PASS (min_order={min(numeric_orders):.2f} all_pairs>={args.order_threshold} finest_residual={finest['linf_residual']:.3e})")
        run_logger.log_event("INFO", "ci_gate", {"status": "pass"})
    else:
        run_logger.log_event("INFO", "ci_gate", {"status": "pass"})
    run_logger.close()
