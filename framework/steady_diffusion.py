"""Steady diffusion solver (Jacobi/SOR) minimal implementation used in tests."""
...
def solve_steady_diffusion(u: np.ndarray, f: np.ndarray, dx: float, dy: float, nu: float,
                           ...
                           progress: bool = False, progress_every: int = 1) -> Tuple[int, float]:
    nx, ny = u.shape
    hx2, hy2 = dx*dx, dy*dy
    denom = 2*(hx2 + hy2)
    inv_nu = 1.0 / nu
    ...
    work = u.copy()  # always allocate (simplifies type reasoning)
    residual = 1e30
    # Dynamic tolerance scaled with grid spacing to encourage finer grids to converge deeper
    # This helps manufactured-solution convergence tests capture spatial truncation error scaling.
    # Allow early convergence based purely on tolerance; test harness now measures truncation order separately.
    min_iter_jacobi = 0
